...
const useExternal = (path?: string, options?: Options) => {
  ...
  useEffect(() => {
    ...
    if (opt?.type === 'css' || (!opt?.type && /(^css!|\.css$)/.test(pathname))) {
      const result = loadCss(path, opt?.css as Partial<HTMLStyleElement> | undefined);
      ref.current = result.ref;
      setStatus(result.status);
    } else if (opt?.type === 'js' || (!opt?.type && /(^js!|\.js$)/.test(pathname))) {
      const result = loadScript(path, opt?.js as Partial<HTMLScriptElement> | undefined);
      ref.current = result.ref;
      setStatus(result.status);
    } else {
      // do nothing
