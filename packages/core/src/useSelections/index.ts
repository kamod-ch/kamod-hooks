...
function useSelections<T>(items: T[], options?: T[] | Options<T>) {
  ...
  if (Array.isArray(options)) {
    defaultSelected = options as T[];
  } else if (isPlainObject(options)) {
    const o = options as Options<T>;
    defaultSelected = (o.defaultSelected ?? defaultSelected) as T[];
    itemKey = o.itemKey ?? itemKey;
  }
  ...
  const selectedMap = useMemo(() => {
    const keyToItemMap = new Map<Key, T>();
